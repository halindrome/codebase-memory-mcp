...
/* #2178: macOS tmp_cleaner unlinks /tmp files whose atime, mtime and ctime are
 * all older than three days, including empty lock files a daemon holds for its
 * whole life. The next participant then creates and locks a fresh inode, so
 * the old lock coordinates nothing. Touch must keep a held file young and
 * report the loss once the path stops naming it. */
TEST(private_file_lock_touch_refreshes_held_file_and_detects_unlink) {
#ifdef _WIN32
    SKIP_PLATFORM("age-based temp cleaning and unlink-while-held are POSIX behaviors");
#else
    private_lock_fixture_t fixture;
    bool started = private_lock_fixture_start(&fixture);
    char path[PRIVATE_LOCK_TEST_PATH_CAP];
    bool path_ok = started && private_lock_path(path, &fixture, "touch.lock");
    cbm_private_file_lock_t *held = NULL;
    cbm_private_file_lock_t *successor = NULL;
    cbm_private_file_lock_status_t acquired =
        path_ok ? cbm_private_file_lock_try_acquire(fixture.directory, "touch.lock",
                                                    CBM_PRIVATE_FILE_LOCK_EX, &held)
                : CBM_PRIVATE_FILE_LOCK_IO;
    const time_t stale = 1000000000;
    struct timespec stale_times[2] = {{.tv_sec = stale}, {.tv_sec = stale}};
    bool aged = acquired == CBM_PRIVATE_FILE_LOCK_OK &&
                futimens(cbm_private_file_lock_native_fd_for_test(held), stale_times) == 0;
    cbm_private_file_lock_status_t linked_touch =
        aged ? cbm_private_file_lock_touch(held) : CBM_PRIVATE_FILE_LOCK_IO;
    struct stat refreshed = {0};
    bool refreshed_ok = linked_touch == CBM_PRIVATE_FILE_LOCK_OK && stat(path, &refreshed) == 0;

    bool unlinked = refreshed_ok && unlink(path) == 0;
    cbm_private_file_lock_status_t unlinked_touch =
        unlinked ? cbm_private_file_lock_touch(held) : CBM_PRIVATE_FILE_LOCK_IO;
    /* The hazard itself: with the path gone, a peer's EX succeeds while the
     * original holder still believes it is exclusive. */
    cbm_private_file_lock_status_t successor_status =
        unlinked ? cbm_private_file_lock_try_acquire(fixture.directory, "touch.lock",
                                                     CBM_PRIVATE_FILE_LOCK_EX, &successor)
                 : CBM_PRIVATE_FILE_LOCK_IO;
    if (successor) {
        (void)cbm_private_file_lock_release(&successor);
    }
    if (held) {
    ...
    ASSERT_EQ(acquired, CBM_PRIVATE_FILE_LOCK_OK);
    ASSERT_TRUE(aged);
    ASSERT_EQ(linked_touch, CBM_PRIVATE_FILE_LOCK_OK);
    ASSERT_TRUE(refreshed_ok);
    ASSERT_TRUE(refreshed.st_atime > stale);
    ASSERT_TRUE(refreshed.st_mtime > stale);
